...
const App = () => {
  ...
  const getDepthPhrase = (depth: number): string => {
    if (depth <= 15) return "very shallow";
    if (depth <= 40) return "shallow";
    if (depth <= 150) return "moderately deep";
        ...
        <>
          <div className="clue-card">
            ...
            <p className="clue-text">
              A <span className="clue-depth">{getDepthPhrase(earthquake.depthKm)}</span> <strong>magnitude {earthquake.magnitude.toFixed(1)}</strong> earthquake struck somewhere on Earth. Guess the nearest major city, then the year.
            </p>
          </div>
